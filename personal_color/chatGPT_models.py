...
def svm_random_search(x_train, y_train, cv):
    ...
    model = RandomizedSearchCV(svc, parameters, n_iter=10, cv=cv, scoring='accuracy', n_jobs=-1, random_state=42)
...
def random_forest_random_search(x_train, y_train, cv):
    ...
    model = RandomizedSearchCV(rf, parameters, n_iter=10, cv=cv, scoring='accuracy', n_jobs=-1, random_state=42)
    model.fit(x_train, y_train)
    return model
...
def gradient_boosting_random_search(x_train, y_train, cv):
    ...
    model = RandomizedSearchCV(gb, parameters, n_iter=10, cv=cv, scoring='accuracy', n_jobs=-1, random_state=42)
    model.fit(x_train, y_train)
    return model
...
def main():
    ...
    for name, model in models.items():
        ...
        print(f"{name} Accuracy: {accuracy_score(y, y_pred)}")
