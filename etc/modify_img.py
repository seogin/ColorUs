...
# Define paths
image_dir = '/Users/seoginhong/Desktop/Personal/ColorUs/ColorUs_Project'
output_dir = '/Users/seoginhong/Desktop/Personal/ColorUs/ab'
no_face_dir = '/Users/seoginhong/Desktop/Personal/ColorUs/no_face_detected'
...
def organize_images(labels, features, original_labels, output_dir, no_face_dir):
    for label, feature, original_label in zip(labels, features, original_labels):
        ...
        if predicted_season is None:
            ...
            print(f"Deleting {label} (original: {original_label}, predicted: {predicted_season})")
            os.remove(label)
...
for image_path in labels:
    ...
    if right_cheek is not None and left_cheek is not None:
        if isinstance(right_cheek, np.ndarray) and isinstance(left_cheek, np.ndarray) and right_cheek.size != 0 and left_cheek.size != 0:
